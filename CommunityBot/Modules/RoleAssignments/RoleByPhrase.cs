...
namespace CommunityBot.Modules.RoleAssignments
{
    ...
    public class RoleByPhrase : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task RbpStatus()
        {
            ...
            var relations = rbp.Relations.Any() ? string.Join("\n", rbp.Relations.Select(r => $"Phrase {r.PhraseIndex} =({r.Type})=> Role {r.RoleIdIndex}")) : "No relations created\nAdd one with `rbp addRelation PHRASE-ID ROLE-ID`";
        ...
        public async Task RbpAddRelation(int phraseIndex, int roleIndex, bool remove = false)
        {
            var type = remove ? RoleRelationType.Remove : RoleRelationType.Add;
            var result = RoleByPhraseProvider.CreateRelation(Context.Guild, phraseIndex, roleIndex, type);
        ...
        public async Task RbpRemoveRelation(int phraseIndex, int roleIndex, bool remove = false)
        {
            var type = remove ? RoleRelationType.Remove : RoleRelationType.Add;
            RoleByPhraseProvider.RemoveRelation(Context.Guild, phraseIndex, roleIndex, type);
            await RbpStatus();
        }
